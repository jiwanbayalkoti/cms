...
@php
    ...
    $baseUrl = route('admin.projects.index');
@endphp
<div class="mb-4 flex flex-wrap gap-2" id="projects-filter-tabs">
    <a href="#" data-status="all" class="project-filter-link px-3 py-1.5 rounded-lg text-sm font-medium transition {{ $statusFilter === 'all' ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">All</a>
    <a href="#" data-status="active" class="project-filter-link px-3 py-1.5 rounded-lg text-sm font-medium transition {{ $statusFilter === 'active' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">Active</a>
    <a href="#" data-status="completed" class="project-filter-link px-3 py-1.5 rounded-lg text-sm font-medium transition {{ $statusFilter === 'completed' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">Completed</a>
</div>

<div id="projects-list-content">
    @include('admin.projects.partials._list', [
        'companiesWithProjects' => $companiesWithProjects ?? collect(),
...
// Project filter: load via AJAX (no full page reload)
document.addEventListener('click', function(e) {
    const link = e.target.closest('.project-filter-link');
    if (!link) return;
    e.preventDefault();
    const status = link.getAttribute('data-status') || 'all';
    const baseUrl = '{{ route("admin.projects.index") }}';
    const sep = baseUrl.indexOf('?') >= 0 ? '&' : '?';
    const url = baseUrl + sep + 'status=' + encodeURIComponent(status) + '&filter_ajax=1';
    ...
    .then(function(data) {
        if (data.html !== undefined) {
            ...
            var cleanUrl = baseUrl + (baseUrl.indexOf('?') >= 0 ? '&' : '?') + 'status=' + encodeURIComponent(data.statusFilter || status);
            if (history.replaceState) {
                history.replaceState(null, '', cleanUrl);
            }
            var activeFilter = data.statusFilter || status;
            if (tabs) {
                tabs.querySelectorAll('.project-filter-link').forEach(function(a) {
                    var s = a.getAttribute('data-status');
                    a.classList.remove('bg-indigo-600', 'text-white', 'bg-green-600', 'bg-blue-600');
