...
@section('title', 'Edit Running Bill')
@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h3 mb-0">Edit Running Bill</h1>
    <a href="{{ route('admin.running-bills.index') }}" class="btn btn-outline-secondary"><i class="bi bi-arrow-left me-1"></i>Back</a>
</div>

<form method="POST" action="{{ route('admin.running-bills.update', $running_bill) }}" id="rbForm">
    @csrf
    @method('PUT')
    <div class="card mb-4">
                ...
                <div class="col-md-6">
                    ...
                    <select name="project_id" class="form-select" required>
                        @foreach($projects as $p)
                            <option value="{{ $p->id }}" {{ $running_bill->project_id == $p->id ? 'selected' : '' }}>{{ $p->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-3">
                    <label class="form-label">Contract No</label>
                    <input type="text" name="contract_no" class="form-control" value="{{ old('contract_no', $running_bill->contract_no) }}">
                </div>
                <div class="col-md-3">
                    <label class="form-label">Bill Date *</label>
                    <input type="date" name="bill_date" class="form-control" value="{{ old('bill_date', $running_bill->bill_date?->format('Y-m-d')) }}" required>
                </div>
                <div class="col-md-12">
                    <label class="form-label">Bill Title *</label>
                    <input type="text" name="bill_title" class="form-control" value="{{ old('bill_title', $running_bill->bill_title) }}" required>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card mb-4">
        <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
            ...
            <div class="d-flex gap-1">
                <button type="button" class="btn btn-sm btn-outline-primary" id="btnLoadFromMb" title="Measurement Book मा add gareko work description, unit, quantity यहाँ ल्याउनुहोस्"><i class="bi bi-journal-text me-1"></i>Load from Measurement Book</button>
            ...
            <div class="table-responsive">
                <table class="table table-bordered mb-0" id="rbItemsTable">
                    ...
                    <tbody id="rbItemsBody">
                        @foreach($running_bill->items as $i => $it)
                        <tr class="rb-item-row">
                            <td>{{ $i + 1 }}</td>
                            <td><textarea name="items[{{ $i }}][description]" class="form-control form-control-sm" rows="2" required>{{ old('items.'.$i.'.description', $it->description) }}</textarea></td>
                            <td><input type="text" name="items[{{ $i }}][unit]" class="form-control form-control-sm" value="{{ old('items.'.$i.'.unit', $it->unit) }}" list="rbUom"></td>
                            <td><input type="number" name="items[{{ $i }}][boq_qty]" class="form-control form-control-sm" step="0.0001" min="0" onchange="calcRow(this)" value="{{ old('items.'.$i.'.boq_qty', $it->boq_qty) }}"></td>
                            <td><input type="number" name="items[{{ $i }}][boq_unit_price]" class="form-control form-control-sm" step="0.01" min="0" onchange="calcRow(this)" value="{{ old('items.'.$i.'.boq_unit_price', $it->boq_unit_price ?? 0) }}"></td>
                            <td><input type="number" name="items[{{ $i }}][this_bill_qty]" class="form-control form-control-sm" step="0.0001" min="0" onchange="calcRow(this)" value="{{ old('items.'.$i.'.this_bill_qty', $it->this_bill_qty) }}" required></td>
                            <td><input type="number" name="items[{{ $i }}][unit_price]" class="form-control form-control-sm" step="0.01" min="0" onchange="calcRow(this)" value="{{ old('items.'.$i.'.unit_price', $it->unit_price) }}" required></td>
                            <td><input type="number" name="items[{{ $i }}][total_price]" class="form-control form-control-sm" step="0.01" readonly value="{{ old('items.'.$i.'.total_price', $it->total_price) }}"></td>
                            <td><input type="number" name="items[{{ $i }}][remaining_qty]" class="form-control form-control-sm" step="0.0001" readonly value="{{ old('items.'.$i.'.remaining_qty', $it->remaining_qty) }}"></td>
                            <td><input type="text" name="items[{{ $i }}][remarks]" class="form-control form-control-sm" value="{{ old('items.'.$i.'.remarks', $it->remarks) }}"></td>
...
@push('scripts')
<script>
let rbRowIndex = {{ $running_bill->items->count() }};
function addRbRow(item) {
    ...
    tr.innerHTML = r.innerHTML.replace(/items\[\d+\]/g, 'items[' + rbRowIndex + ']');
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    if (btnLoad && projectSelect) {
        btnLoad.addEventListener('click', function(){
            ...
            if (!pid) { alert('Project select गर्नुहोस्।'); return; }
            ...
            fetch('{{ route('admin.running-bills.measurement-book-items') }}?project_id=' + encodeURIComponent(pid), { headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' } })
                ...
                .then(function(data){
                    ...
                    if (updated > 0 && added > 0) {
                        ...
                        msg = added + ' item(s) Measurement Book बाट ल्याइयो।';
                    } else {
