...
@section('title', 'Edit Construction Material')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h3 mb-0">Edit Construction Material</h1>
        ...
        <form action="{{ route('admin.construction-materials.update', $material) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <select name="material_category" class="form-select">
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->name }}" {{ old('material_category', $material->material_category) === $category->name ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <select name="supplier_name" class="form-select">
                        ...
                        @foreach($suppliers as $supplier)
                            <option value="{{ $supplier->name }}" {{ old('supplier_name', $material->supplier_name) === $supplier->name ? 'selected' : '' }}>
                                {{ $supplier->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="supplier_contact" class="form-control" value="{{ old('supplier_contact', $material->supplier_contact) }}">
                </div>
                <div class="col-md-4">
                    <label class="form-label">Bill Number</label>
                    <input type="text" name="bill_number" class="form-control" value="{{ old('bill_number', $material->bill_number) }}">
                </div>

                <div class="col-md-4">
                    <label class="form-label">Bill Date</label>
                    <input type="date" name="bill_date" class="form-control" value="{{ old('bill_date', optional($material->bill_date)->format('Y-m-d')) }}">
                </div>
                <div class="col-md-4">
                ...
                <div class="col-md-4">
                    ...
                    <select name="payment_mode" class="form-select">
                        ...
                        @foreach($paymentModes as $paymentMode)
                            <option value="{{ $paymentMode->name }}" {{ old('payment_mode', $material->payment_mode) === $paymentMode->name ? 'selected' : '' }}>
                                {{ $paymentMode->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <select name="purchased_by_id" class="form-select">
                        ...
                        @foreach($purchasedBies as $purchasedBy)
                            <option value="{{ $purchasedBy->id }}" {{ old('purchased_by_id', $material->purchased_by_id) == $purchasedBy->id ? 'selected' : '' }}>
                                {{ $purchasedBy->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <input type="date" name="delivery_date" class="form-control" value="{{ old('delivery_date', optional($material->delivery_date)->format('Y-m-d')) }}">
                </div>
                <div class="col-md-4">
                    <label class="form-label">Delivery Site</label>
                    <input type="text" name="delivery_site" class="form-control" value="{{ old('delivery_site', $material->delivery_site) }}">
                </div>
                <div class="col-md-4">
                    <label class="form-label">Delivered By</label>
                    <input type="text" name="delivered_by" class="form-control" value="{{ old('delivered_by', $material->delivered_by) }}">
                </div>
    ...
    <select name="project_id" class="form-select">
        ...
        @endforeach
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="work_type" class="form-control" value="{{ old('work_type', $material->work_type) }}" placeholder="e.g., PCC, Soling, Masonry">
                </div>

                <div class="col-md-12">
                    <label class="form-label">Usage Purpose</label>
                    <textarea name="usage_purpose" rows="3" class="form-control">{{ old('usage_purpose', $material->usage_purpose) }}</textarea>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <select name="status" class="form-select">
                        ...
                        <option value="Damaged" {{ old('status', $material->status) === 'Damaged' ? 'selected' : '' }}>Damaged</option>
                    </select>
                </div>
                <div class="col-md-4">
                    <label class="form-label">Approved By</label>
                    <input type="text" name="approved_by" class="form-control" value="{{ old('approved_by', $material->approved_by) }}">
                </div>
                <div class="col-md-4">
                    <label class="form-label">Approval Date</label>
                    <input type="date" name="approval_date" class="form-control" value="{{ old('approval_date', optional($material->approval_date)->format('Y-m-d')) }}">
                </div>
