...
<div class="mb-6 flex justify-between items-center">
    ...
    <a href="{{ route('admin.positions.create') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg transition duration-200">
        Add New Position
    </a>
</div>
            ...
            @forelse($positions as $position)
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <a href="{{ route('admin.positions.show', $position) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">View</a>
                        <a href="{{ route('admin.positions.edit', $position) }}" class="text-yellow-600 hover:text-yellow-900 mr-3">Edit</a>
                        <form action="{{ route('admin.positions.destroy', $position) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this position?');">
                            @csrf
                            @method('DELETE')
            ...
            @empty
                <tr>
                    <td colspan="6" class="px-6 py-4 text-center text-sm text-gray-500">
                        No positions found. <a href="{{ route('admin.positions.create') }}" class="text-indigo-600 hover:text-indigo-900">Create one now</a>
                    </td>
                </tr>
            @endforelse
        </tbody>
