...
@section('title', 'Edit Measurement Book')
@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h3 mb-0">Edit Measurement Book</h1>
    <a href="{{ route('admin.measurement-books.index') }}" class="btn btn-outline-secondary"><i class="bi bi-arrow-left me-1"></i>Back</a>
</div>

<form method="POST" action="{{ route('admin.measurement-books.update', $measurement_book) }}" id="mbForm">
    @csrf
    @method('PUT')
    <div class="card mb-4">
                ...
                <div class="col-md-6">
                    ...
                    <select name="project_id" class="form-select" required>
                        @foreach($projects as $p)
                            <option value="{{ $p->id }}" {{ $measurement_book->project_id == $p->id ? 'selected' : '' }}>{{ $p->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-3">
                    <label class="form-label">Contract No</label>
                    <input type="text" name="contract_no" class="form-control" value="{{ old('contract_no', $measurement_book->contract_no) }}">
                </div>
                <div class="col-md-3">
                    <label class="form-label">Measurement Date *</label>
                    <input type="date" name="measurement_date" class="form-control" value="{{ old('measurement_date', $measurement_book->measurement_date?->format('Y-m-d')) }}" required>
                </div>
                <div class="col-md-6">
                    <label class="form-label">Title (optional)</label>
                    <input type="text" name="title" class="form-control" value="{{ old('title', $measurement_book->title) }}">
                </div>
                <div class="col-md-6">
                    <label class="form-label">L, W, H Unit</label>
                    <select name="dimension_unit" id="dimensionUnit" class="form-select">
                        @php $dimVal = old('dimension_unit', $measurement_book->dimension_unit ?? 'ft'); if ($dimVal === 'ft_in') $dimVal = 'ft'; if ($dimVal === 'm_cm') $dimVal = 'm'; @endphp
                        @foreach($dimensionUnits as $key => $label)
                            <option value="{{ $key }}" {{ $dimVal == $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered mb-0" id="mbItemsTable">
                    ...
                    <tbody id="mbItemsBody">
                        ...
                        <tr class="mb-item-row mb-main-work" data-row-index="{{ $itemIndex }}">
                            <td class="sn-cell">{{ $mainItem->sn }}</td>
                            <td><textarea name="items[{{ $itemIndex }}][works]" class="form-control form-control-sm" rows="2" required placeholder="Main work description">{{ old('items.'.$itemIndex.'.works', $mainItem->works) }}</textarea></td>
                            <td class="main-work-field"><input type="number" name="items[{{ $itemIndex }}][no]" class="form-control form-control-sm" value="{{ old('items.'.$itemIndex.'.no', $mainItem->no) }}" step="0.0001" min="0" onchange="calcQty(this)"></td>
                            <td class="main-work-field"><input type="number" name="items[{{ $itemIndex }}][length_ft]" class="form-control form-control-sm" value="{{ old('items.'.$itemIndex.'.length_ft', $mainItem->length_ft) }}" step="0.0001" min="0" onchange="calcQty(this)"></td>
                            <td class="main-work-field"><input type="number" name="items[{{ $itemIndex }}][breadth_ft]" class="form-control form-control-sm" value="{{ old('items.'.$itemIndex.'.breadth_ft', $mainItem->breadth_ft) }}" step="0.0001" min="0" onchange="calcQty(this)"></td>
                            <td class="main-work-field"><input type="number" name="items[{{ $itemIndex }}][height_ft]" class="form-control form-control-sm" value="{{ old('items.'.$itemIndex.'.height_ft', $mainItem->height_ft) }}" step="0.0001" min="0" onchange="calcQty(this)"></td>
                            <td class="main-work-field"><input type="number" name="items[{{ $itemIndex }}][quantity]" class="form-control form-control-sm" value="{{ old('items.'.$itemIndex.'.quantity', $mainItem->quantity) }}" step="0.0001" min="0" readonly placeholder="auto"></td>
                            <td><input type="number" name="items[{{ $itemIndex }}][total_qty]" class="form-control form-control-sm main-total-qty" value="{{ old('items.'.$itemIndex.'.total_qty', $mainItem->total_qty) }}" step="0.0001" min="0" readonly placeholder="auto"></td>
                            <td class="main-work-field"><input type="text" name="items[{{ $itemIndex }}][unit]" class="form-control form-control-sm" value="{{ old('items.'.$itemIndex.'.unit', $mainItem->unit) }}" list="uomList" onchange="calcQty(this)"></td>
                            <td>
                                ...
                                <button type="button" class="btn btn-sm btn-outline-danger" onclick="removeMbRow(this)" title="Remove"><i class="bi bi-trash"></i></button>
                            </td>
                        </tr>
                        @php $itemIndex++; @endphp
                        @foreach($mainItem->children as $subItem)
                        <tr class="mb-item-row mb-sub-work" data-row-index="{{ $itemIndex }}" data-parent-index="{{ $itemIndex - 1 }}">
                            <td class="sn-cell"></td>
                            <td class="works-cell"><textarea name="items[{{ $itemIndex }}][works]" class="form-control form-control-sm" rows="2" placeholder="Sub work description">{{ old('items.'.$itemIndex.'.works', $subItem->works) }}</textarea></td>
                            <td><input type="number" name="items[{{ $itemIndex }}][no]" class="form-control form-control-sm" value="{{ old('items.'.$itemIndex.'.no', $subItem->no) }}" step="0.0001" min="0" onchange="calcQty(this)"></td>
                            <td><input type="number" name="items[{{ $itemIndex }}][length_ft]" class="form-control form-control-sm" value="{{ old('items.'.$itemIndex.'.length_ft', $subItem->length_ft) }}" step="0.0001" min="0" onchange="calcQty(this)"></td>
                            <td><input type="number" name="items[{{ $itemIndex }}][breadth_ft]" class="form-control form-control-sm" value="{{ old('items.'.$itemIndex.'.breadth_ft', $subItem->breadth_ft) }}" step="0.0001" min="0" onchange="calcQty(this)"></td>
                            <td><input type="number" name="items[{{ $itemIndex }}][height_ft]" class="form-control form-control-sm" value="{{ old('items.'.$itemIndex.'.height_ft', $subItem->height_ft) }}" step="0.0001" min="0" onchange="calcQty(this)"></td>
                            <td><input type="number" name="items[{{ $itemIndex }}][quantity]" class="form-control form-control-sm sub-qty" value="{{ old('items.'.$itemIndex.'.quantity', $subItem->quantity) }}" step="0.0001" min="0" readonly placeholder="auto"></td>
                            <td><input type="number" name="items[{{ $itemIndex }}][total_qty]" class="form-control form-control-sm" value="{{ old('items.'.$itemIndex.'.total_qty', $subItem->total_qty) }}" step="0.0001" min="0" readonly placeholder="auto"></td>
                            <td><input type="text" name="items[{{ $itemIndex }}][unit]" class="form-control form-control-sm" value="{{ old('items.'.$itemIndex.'.unit', $subItem->unit) }}" list="uomList" onchange="calcQty(this)"></td>
                            <td>
                                <input type="hidden" name="items[{{ $itemIndex }}][parent_id]" value="{{ $itemIndex - 1 }}">
                                <button type="button" class="btn btn-sm btn-outline-danger" onclick="removeMbRow(this)" title="Remove"><i class="bi bi-trash"></i></button>
                            </td>
                        </tr>
                        @php $itemIndex++; @endphp
    ...
    <div class="d-flex gap-2">
        ...
        <a href="{{ route('admin.measurement-books.index') }}" class="btn btn-secondary"><i class="bi bi-x-lg me-1"></i>Cancel</a>
    </div>
</form>
...
@push('scripts')
<script>
let mbRowIndex = {{ $measurement_book->items->count() }};
...
function addSubWorkRow(btn) {
    ...
    // Use data-row-index instead of DOM index to get the correct parent index
    ...
    let nextRow = mainRow.nextElementSibling;
    while (nextRow && nextRow.classList.contains('mb-sub-work')) {
        const nextParentIndex = parseInt(nextRow.getAttribute('data-parent-index') || '-1');
...
function renumberMbRows() {
    ...
    // Get all rows in DOM order
    const allRows = Array.from(document.querySelectorAll('#mbItemsBody .mb-item-row'));
    
    // First pass: store parent row references using DOM structure (more reliable)
    const parentMap = new Map();
    allRows.forEach((tr) => {
        if (tr.classList.contains('mb-sub-work')) {
            // Find parent by going backwards in DOM until we find a main work
            let currentRow = tr.previousElementSibling;
            while (currentRow && !currentRow.classList.contains('mb-main-work')) {
                currentRow = currentRow.previousElementSibling;
            }
            if (currentRow && currentRow.classList.contains('mb-main-work')) {
                parentMap.set(tr, currentRow);
            }
        }
    });
    
    // Second pass: renumber all rows and update parent_id references
    allRows.forEach((tr, newIndex) => {
...
function updateMainWorkTotals() {
    document.querySelectorAll('#mbItemsBody .mb-main-work').forEach(mainRow => {
        ...
        // Sum all sub-work quantities - check both parent_id input and data-parent-index attribute
        let nextRow = mainRow.nextElementSibling;
        while (nextRow && nextRow.classList.contains('mb-sub-work')) {
            const parentIdInput = nextRow.querySelector('input[name*="[parent_id]"]');
            const parentId = parentIdInput ? parseInt(parentIdInput.value || '-1') : -1;
            const dataParentIndex = parseInt(nextRow.getAttribute('data-parent-index') || '-1');
            
            // Check if this sub-work belongs to this main work (check both parent_id and data-parent-index)
            if (parentId === mainIndex || dataParentIndex === mainIndex) {
                hasSubWorks = true;
                ...
                // Not a sub-work of this main work, stop looking
                break;
            }
        }
